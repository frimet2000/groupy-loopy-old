        ...
        >
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 md:space-y-6">
            <TabsList className="bg-white p-1 rounded-xl border shadow-sm w-full grid grid-cols-4 h-auto">
              <TabsTrigger 
                value="upcoming" 
                className="gap-1 md:gap-2 data-[state=active]:bg-emerald-50 data-[state=active]:text-emerald-700 flex-col md:flex-row py-2 text-[10px] md:text-sm"
              >
                <Calendar className="w-3.5 h-3.5 md:w-4 md:h-4" />
                <span>{language === 'he' ? 'קרובים' : 'Upcoming'}</span>
                {upcomingTrips.length > 0 && (
                  <span className="bg-emerald-600 text-white text-[9px] md:text-xs px-1 md:px-2 py-0.5 rounded-full absolute -top-1 -right-1 md:static md:ml-1">
              ...
              <TabsTrigger 
                value="joined" 
                className="gap-1 md:gap-2 data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700 flex-col md:flex-row py-2 text-[10px] md:text-sm"
              >
              ...
              <TabsTrigger 
                value="organized" 
                className="gap-1 md:gap-2 data-[state=active]:bg-purple-50 data-[state=active]:text-purple-700 flex-col md:flex-row py-2 text-[10px] md:text-sm"
              >
              ...
              <TabsTrigger 
                value="saved" 
                className="gap-1 md:gap-2 data-[state=active]:bg-amber-50 data-[state=active]:text-amber-700 flex-col md:flex-row py-2 text-[10px] md:text-sm relative"
              >
                <Bookmark className="w-3.5 h-3.5 md:w-4 md:h-4" />
                <span>{language === 'he' ? 'שמורים' : 'Saved'}</span>
                {savedTrips.length > 0 && (
                  <span className="bg-amber-600 text-white text-[9px] md:text-xs px-1 md:px-2 py-0.5 rounded-full absolute -top-1 -right-1 md:static md:ml-1">
                    {savedTrips.length}
                  </span>
                )}
              </TabsTrigger>
